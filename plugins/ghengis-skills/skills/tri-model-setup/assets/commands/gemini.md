...
- Otherwise: strip a leading `--pro` flag (sets `-Model pro`; default is `-Model flash`), then run via PowerShell:
  `powershell -NoProfile -File "$HOME/.claude/scripts/agy-call.ps1" -Prompt "<the question>" -Model <flash|pro>`
...
- If auth fails: tell the user to run `agy` once in a terminal and sign in with the PERSONAL Google account (AI Pro), not Workspace.
